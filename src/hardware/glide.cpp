...
static void write_gl_wrapped(io_port_t port, io_val_t val, io_width_t width)
{
    (void)port;
    (void)width;
    ...
    MEM_BlockRead32(PhysicalMake(glsegment,0), param, 80);
    process_msg(val);
...
class GLIDE_PageHandler final : public PageHandler {
private:
...
public:
    ...
    GLIDE_PageHandler(HostPt addr, PhysPt phyaddr = GLIDE_LFB) : base_addr{}, lin_addr{}, lfb_addr{}, locked{} {
	...
	for(int i = 0; i < GLIDE_BUFFERS; i++) {
	    ...
	    lfb_addr[i] = (addr == NULL) ? NULL : (addr - (Bitu)base_addr[i]);
	}
	...
	addr = addr - (Bitu)base_addr[buffer];	/* Calculate offset for current buffer */

	if(addr != lfb_addr[buffer]) {
	    lfb_addr[buffer] = addr;
#if LOG_GLIDE
	    LOG_MSG("Glide:LFB for buffer %d offset set to 0x%p (addr: 0x%p), clear TLB", (int)buffer, lfb_addr[buffer], addr+(Bitu)base_addr[buffer]);
#endif
...
class GLIDE final : public Module_base {
private:
    // Glide port
    Bitu glide_base;
public:
	...
	if(texmem == NULL) {
	    LOG_F(INFO, "Glide:Unable to allocate texture memory, glide disabled");
	    return;
	}

	glide.lfb_pagehandler = new GLIDE_PageHandler((HostPt)texmem);
	if(!glide.lfb_pagehandler) {
	    LOG_F(INFO, "Glide:Failed to install page handler, glide disabled!");
	...
	if(fn_pt == NULL) {
	    LOG_F(INFO, "Glide:Unable to allocate memory, glide disabled");
	    free(texmem); texmem = NULL;
	    return;
	}
...
#if LOG_GLIDE
	    if(fn_pt[i] == NULL) {
		LOG_F(INFO, "Glide:Warning, unable to load %s from glide2x", grTable[i].name);
	    }
#endif
	}
	...
	IO_RegisterReadHandler(glide_base,read_gl_wrapped,io_width_t::dword);
	IO_RegisterWriteHandler(glide_base,write_gl_wrapped,io_width_t::dword);

	ostringstream temp;
	temp << hex << glide_base;
	AUTOEXEC_SetVariable("GLIDE", temp.str());
	...
	if(glide_base) {
	    IO_FreeReadHandler(glide_base,io_width_t::dword);
	    IO_FreeWriteHandler(glide_base,io_width_t::dword);
	}
...
void GLIDE_Init() {
    Section_prop* voodoo_sec = dynamic_cast<Section_prop*>(control->GetSection("voodoo"));
    if (!voodoo_sec || !voodoo_sec->Get_bool("glide")) {
        return;
    }
    test = new GLIDE(voodoo_sec);
    GLIDE_Register();
}
...
*/
	...
	k = 0;
	do {
	    mem_writeb(param[1]++, (uint8_t)filename[k++]);
	} while(filename[k] != '\0');
	...
	if (buffer >= GLIDE_BUFFERS) {
	    LOG_MSG("Glide:Invalid buffer passed in grLfbLock (%lu)", (unsigned long)buffer);
	    return;
	}
	...
	if(dblfbinfo.strideInBytes > 1600*4) LOG_MSG("Glide:WARNING LFB stride (%d) is larger than max supported: %d!", (int)dblfbinfo.strideInBytes, 1600*4);
...
*/
	...
	uint32_t val_j;
	SDL_memcpy(&val_j, &tmp, 4);
	mem_writed(ret, SDL_SwapLE32(val_j));
	ret_value = G_OK;
	break;
    }
/*
...
*/
	...
	for(k = 0; k < param[13]; k++) {
		...
		if(c > 0xE0) {
			...
			// tlut is FxU16*
			src[dcount] = mem_readw(param[16]+((Bitu)mem_readb(param[8]+scount+1)<<1));
			dcount++;
		    }
		    scount += 2;

		} else {
		    src[dcount] = mem_readw(param[16]+((Bitu)c<<1));
		    dcount++; scount++;
		}
	    }
